 ...
 * File: loaddb.cc
 * Author: battou
 * Version: $Id: test3.cc,v 1.11 1998/08/06 04:04:47 bilal Exp $
 * Purpose: Test the reading of a database from a file.
 */
#ifndef LINT
static char const _loaddb_cc_rcsid_[] =
"$Id: test3.cc,v 1.11 1998/08/06 04:04:47 bilal Exp $";
#endif
...
void HelloTerm::Absorb(Visitor * v) 
{
  ...
  if (vt2 && vt1.Is_A(vt2)) {
    ...
    if (((ElectionVisitor *)v)->GetPurpose() == ElectionVisitor::ToOriginateANodalinfo) {
      ...
      u_char node[22];
      u_char * pg = 0;
      bcopy((void *)ev->GetNode()->GetNID(), node, 22);
      if (ev->GetPGL()) {
        pg = new u_char[22];
        bcopy((void *)ev->GetPGL()->GetNID(), pg, 22);
      } 
      int result = 1;
      u_char co[] = "\x60\xa0\x47\x00\x05\x80\xff\xde\x00\x00\x01\x00\x00\x04\x02\xff\x1c\x06\x92\x00\x01\x00";
      u_char flag[] = "\x08";
      result = memcmp(ev->GetNode()->GetNID(), co, 22);
      ig_nodal_info_group * nig = 0;
      if(result == 0)
      nig = new ig_nodal_info_group(&node[0], (u_char)ev->GetPriority(), flag[0], pg);
      else 
...
StartTimer::StartTimer(Terminal *t, Visitor *v) :
  TimerHandler(t,0), _t(t),_v(v) { }
StartTimer::~StartTimer() { }

void StartTimer::Callback(void) {  _t->Inject(_v);  }
...
void main(int argc, char **argv)
{
  ...
  // argv[0] counts at 1 arg
  if (argc < 2) {
    cout << "Usage: " << argv[0] << " db1 db2 dbc1 dbc2 666 time 0 output " << endl;
    exit(1);
  }

  if (argc >= 5) {
    strcpy(path1, argv[1]);  // DB1
    strcpy(path2, argv[2]);  // DB2
    strcpy(path3, argv[3]);  // DBC1
    strcpy(path4, argv[4]);  // DBC2
  } else {
    if (!getcwd(path1, 80)) {
      cout << "cant get CWD \n";
      exit(1);
  ...
  Conduit * unused3 = InitDB(fp, "DB1", db1, nn1, npn1, nppgl1, 1);
  ...
  Conduit * unused4 = InitDB(fp, "DB2", db2, nn2, npn2, nppgl2, 1);
  ...
  Conduit * unused5 = InitDB(fp, "DB1", db1, nn1, npn1, nppgl1, 1);
  ...
  Conduit * unused6 = InitDB(fp, "DB2", db2, nn2, npn2, nppgl2, 1);
  ...
  cout << "Simulation ended (\abeep!)" << endl;
}
