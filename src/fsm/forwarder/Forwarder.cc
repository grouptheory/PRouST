...
"$Id: Forwarder.cc,v 1.198 1999/03/04 21:30:33 mountcas Exp $";
#endif
    ...
    _DATA_DELAY(0.0), _CTRL_DELAY(0.01)
{
  ...
  AddPermission("*", new ForwarderDefaultInterface(this) );
}
				      ...
				      int iport, int ivpi, int ivci)
{
  ...
  switch (msg->type()) {
	...
	// Check to see if we are the final destination (if so we won't see the setup again)
    ...
    case header_parser::release_msg:
      // See if we are the originator of the Call
	...
	// lookup the bindings for this call, move the binding into
	// the _setup_in_progress_Map

	// since this came from outside, it MUST NOT be in the
	// _setup_in_progress_Map
	assert( _setup_in_progress_Map.lookup( fv) == 0 );
	// and it MUST be in the _Port2_CREF2Fab_Map
	dic_item di = _Port2_CREF2Fab_Map.lookup( iport );
	assert( di != 0 );

	dictionary<unsigned int, Fab *> * CREF2Fab = _Port2_CREF2Fab_Map.inf( di );
	di = CREF2Fab->lookup( icref );
	assert( di != 0 );

	Fab * out_fab = CREF2Fab->inf(di);
	// If this isn't the way we came in ...
	if ( out_fab->GetPort() != fv->GetInPort() ) {
	  Fab * cpy = new Fab( *out_fab );
	  cpy->SetAddr( out_fab->GetAddr()->copy() );
	  _setup_in_progress_Map.insert( fv, cpy );
	} else {
	  // Now that we've found the correct Fabric we must locate the inverse of it ...
	  di = _FabFabMap.lookup(out_fab);
	  assert( di != 0 );
	  list<Fab *> * flist = _FabFabMap.inf(di);

	  int count = flist->length();
	  assert( count == 1 );  // For now only handle p2p

	  Fab * in_fab = flist->head();

	  Fab * cpy = new Fab( *in_fab );
	  cpy->SetAddr( in_fab->GetAddr()->copy() );
	  _setup_in_progress_Map.insert( fv, cpy );
	}
	if (!UnBind( iport, icref )) {
	  DIAG("fsm.forwarder", DIAG_FATAL, cout << *_LocNodeID <<
	       ": Failure to unbind pending information for Port " << 
	       iport << " CREF 0x" << hex << icref << dec << endl);
	} 
      }
...
{ 
  ...
  switch (msg->type()) 
	...
	if ( di != 0 ) {
	  // If it comes back from Control as a release we can remove this binding
...
void Forwarder::ForwardRelease(FastUNIVisitor *& release)
{
  ...
  dic_item di = _setup_in_progress_Map.lookup(release);
