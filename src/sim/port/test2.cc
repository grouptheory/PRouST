 ...
 * File: loaddb.cc
 * Author: battou
 * Version: $Id: test2.cc,v 1.3 1998/08/06 04:05:36 bilal Exp $
 * Purpose: Test the reading of a database from a file.
 */

#ifndef LINT
static char const _loaddb_cc_rcsid_[] =
"$Id: test2.cc,v 1.3 1998/08/06 04:05:36 bilal Exp $";
#endif
...
main(int argc, char *argv[])
{
  ...
  DiagLevel(FSM_HELLO, DIAG_ERROR);
  ...
  PurifyingLinkState * l=new PurifyingLinkState(p1,p2);
  ...
  // join side B of NPs to side A of Databases
  Join( B_half(npc1), A_half(dc1) );
  Join( B_half(npc2), A_half(dc2) );

  // join side A half of Hellos to A half of NPs
  Join( A_half(hc1), A_half(npc1) );
  Join( A_half(hc2), A_half(npc2) );

  // join B half of Hellos to A half of linkState and B half of link State
  Join(B_half(hc1), A_half(qc1));
  Join(B_half(qc1), A_half(lc));
